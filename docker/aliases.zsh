...
alias dockerps='docker ps --no-trunc --format "table {{printf \"%.12s\" .ID}}\t{{.Names}}\t{{.Status}}\t{{.Command}}\t{{.Image}}"'
alias dockerpsae='docker ps -a --filter "status=exited" --no-trunc --format "table {{printf \"%.12s\" .ID}}\t{{.Names}}\t{{.Status}}\t{{.Command}}\t{{.Image}}"'
